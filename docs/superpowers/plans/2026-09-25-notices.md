...
1. **A done notice's timer removing a later notice that took its key.** "Folder saved", then a failure a second later under the same key: the failure must still be on screen after the four seconds the success was given. Timers belong to a notice's `id`, never its `key`. Test in Task 1.
2. **The same failure, repeated.** Pressing a failing button five times must leave one notice, not a stack of five. Test in Task 1.
3. **A long unbroken text** — a path, the driver's own words. It must wrap inside the notice and never run off the right edge of the window. Test in Task 1.
4. **A dialog opened while a notice shows.** Escape must still close the dialog, keep the screen, and leave the notice. Test in Task 2.
5. **The empty part of the corner.** The area the notices are stacked in must not swallow clicks meant for what is underneath — only a notice itself takes the pointer. Test in Task 1.
