...
- Modify: `tests/test_interface.py` (the mock's `set_recordings_dir` and `set_output_device`; a new section `[12e]`)
...
```python
        print("\n[12e] Notices: what happened goes in the corner")
...
Expected: the run stops in `[12e]` with a Playwright `TimeoutError` waiting for `[data-notice='done']:has-text('No new interfaces')` — there is no notice yet.
...
Run: `cd ui && npm run build && cd .. && python tests/test_interface.py 2>&1 | sed -n '/\[12e\]/,/^\[/p'`
Expected: every line in `[12e]` starts `  ok`.
...
- Modify: `tests/test_interface.py` (the mock's `get_rehearsal` and `recover_draft`; a new section `[12f]`)
- Modify: `CHANGELOG.md`
...
```python
        print("\n[12f] History and Drafts say what failed in the corner")
...
Run: `cd ui && npm run build && cd .. && python tests/test_interface.py 2>&1 | sed -n '/\[12f\]/,/^\[/p'`
Expected: every line in `[12f]` starts `  ok`. Then the whole run: `0` lines with `  FAIL`, last line `Interface: screens render, the flow works, no console errors.`
