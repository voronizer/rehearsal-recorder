...
2. **The same failure, repeated.** A failing action done again leaves one notice, not a stack. Task 1, test "a second fallback takes the first one's place" — the output change raises its warning without clearing first, so only replacement by key keeps it to one ("the same failure again is one notice, not two" passes even without it, since applyDir clears its slot first).
...
To see only what a new section printed: `python tests/test_interface.py 2>&1 | sed -n '/\[12k\]/,/^\[/p'` (change the section tag).
...
Run: `cd ui && npm run build && cd .. && python tests/test_interface.py 2>&1 | sed -n '/\[12k\]/,/^\[/p'`
...
Run: `cd ui && npm run build && cd .. && python tests/test_interface.py 2>&1 | sed -n '/\[12l\]/,/^\[/p'`
