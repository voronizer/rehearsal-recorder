...
committed — it is build output — so a fresh clone needs this once before
`app.py` will start. Run without it and the app says so instead of opening an
empty window.

Restart `app.py` after a Python change. There is no reload; the audio engine
holds open streams and file handles, and restarting is both faster and less
...
(`DEV_SERVER_PORT` in `app.py`) in this mode rather than a random one: a
config file cannot be told a number that is picked at startup.
...
**`./run-debug.sh`** (macOS) runs the app with the allocator checking itself
on every operation. Slow, but a memory fault lands on the bad write instead
of on some innocent allocation later.
